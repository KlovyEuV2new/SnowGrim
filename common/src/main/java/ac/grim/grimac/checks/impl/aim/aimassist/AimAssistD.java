...
@CheckData(name = "AimAssistD",experimental = true)
public class AimAssistD extends Check implements RotationCheck {
    private final List<Double> samplesYaw = Lists.newArrayList();
    private final List<Double> samplesPitch = Lists.newArrayList();
    private double buffer = 0.0;
    public AimAssistD(@NotNull GrimPlayer player) {
        super(player);
    }
    @Override
            ...
            handle:
            {
                ...
                if (samplesYaw.size() + samplesPitch.size() == 60) {
                    ...
                    if (intercepts > 1.4 && slope > 0.0 && fails > 15 && outliersX < 10 && outliersY < 10) {
                       // if (++buffer > 3) {
                            flagAndAlert(String.format("intercepts=%.3f, slope=%.3f", intercepts, slope));
                        //}
                    } else buffer = 0.15;
                    samplesYaw.clear();
                    samplesPitch.clear();
