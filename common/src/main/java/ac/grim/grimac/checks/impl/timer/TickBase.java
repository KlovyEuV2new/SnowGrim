...
public class TickBase extends Check implements PacketCheck {
    ...
    public TickBase(GrimPlayer player) {
        super(player);
    }
    ...
    public void doCheck(final PacketReceiveEvent event) {
        if (timerBalanceRealTime > System.nanoTime()) {
            ...
            boolean valid = PacketEvents.getAPI().getServerManager().getVersion().isOlderThan(ServerVersion.V_1_9) && player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_9);
            if (flagAndAlert("boost: " + lostMS) && valid && lostMS < minmbalance ) {
                // Cancel the packet
