...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    private void blockMovementsUntilResync(boolean simulateNextTickPosition, boolean isResync,boolean isDown) {
        ...
        if (simulateNextTickPosition) {
            Vector3dm collide = Collisions.collide(player,isDown ? 0.000000001 : clientVel.getX(),isDown ? -7.0 : clientVel.getY() /* clientVel.getY()*/, isDown ? 0.000000001 :clientVel.getZ());
    ...
    public boolean teleportBack() {
        if (this.requiredSetBack == null) {
            ...
            if (this.isPendingSetback()) {
                ...
                Vector3d position = new Vector3d(this.player.lastX, this.player.lastY, this.player.lastZ);
                SetBackData data = new SetBackData(new TeleportData(position, new Vector3d(0.0D, 0.0D, 0.0D), new RelativeFlag(0b11000), this.player.lastTransactionSent.get(), 0), this.player.yaw, this.player.pitch, new Vector3dm(0, 0, 0), this.player.inVehicle(), false);
